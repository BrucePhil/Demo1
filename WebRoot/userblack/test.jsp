...
<html>
  <head>
	...
	<link rel="stylesheet" type="text/css" href="csss/reset_y.css">
<style>
body {
...
.impress-enabled .refresh .list_show img, .impress-enabled .step.active.refresh .list_show img {
	...
	<script id="jquery_183" type="text/javascript" class="library" src="jss/jquery-1.8.3.min.js"></script>
<script> 
   (function() {
     if (! 
     /*@cc_on!@*/
  ...
  <div class="bg"><img src="imagess/bg04.jpg" width="100%" /></div>
	...
	<div id="impress">
		<div id="timeList">
			...
			Calendar c = Calendar.getInstance();
			 ...
			 %>
				<div class="year2012"><%=year %></div>
				
			</div>
			<%
			DBhelperQ db=new DBhelperQ();
			ArrayList<userInfo> arr1=db.query_userid(session.getAttribute("users").toString());
			userInfo ui=arr1.get(0);
			 %>
			<%
			int a=0;
			System.out.println();
			ArrayList<photoinfo>arr=db.query_photoid(ui.getUser_id());
		for(int i=0;i<arr.size();i++){
			photoinfo ph=arr.get(i);
			 %>
			<div class="timeList_item step" data-x="<%=a+=150 %>" data-y="0" id="<%= i %>">
			
				<div class="circle">
				<font size="1"><%= ph.getPhototime()%></font>
				</div>
				<div class="list_show " >
					<img src="photo/<%=ph.getPhotoname() %>" width="500">
				</div>
			</div>
<%} %>
			</div>
...
<div class="gotop"> <a href="#top"><img src="imagess/top.png" title="回顶部"/></a></div>

<script type="text/javascript" src="jss/impress.js"></script>
...
//预加载图片
new Image().src = "imagess/bg04.jpg";
new Image().src = "imagess/bg01.jpg";
new Image().src = "imagess/bg02.jpg";
new Image().src = "imagess/bg03.jpg";
...
$(function(){			
	...
	if(!$.browser.msie){
		if($.browser.safari)
			bgMusic = new Audio('music/aaa.mp3');
		else
